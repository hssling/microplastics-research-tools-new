# This cell creates a Python script file that, when run locally, will generate
# an animated MP4 video of the lifespan gene-expression clock with narration
# (female voice). The script uses matplotlib to create frames, moviepy to
# assemble video, and pyttsx3 for offline TTS. It does NOT generate the
# heavy video here; it only writes the runnable script to disk so you can
# run it locally where dependencies are available.
...
file_path = "gene_expression_animation.py"
with open(file_path, "w", encoding="utf-8") as f:
    f.write(script)

print(f"Wrote script to: {file_path}")
print("To run it locally: python /mnt/data/gene_expression_animation.py")
